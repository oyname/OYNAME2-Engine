...
namespace
{
    struct MovingLight
    {
        ...
        float radiusXZ = 2.0f, speed = 1.0f, phase = 0.0f, bobAmp = 0.35f, bobSpeed = 1.5f;
    ...
    float Frand01() { return static_cast<float>(std::rand()) / static_cast<float>(RAND_MAX); }
    float FrandRange(float a, float b) { return a + (b - a) * Frand01(); }

    void MakeRenderable(LPENTITY& out, LPMESH mesh, LPMATERIAL mat, const char* tag,
        float px, float py, float pz, float sx, float sy, float sz,
        float pitchDeg = 0.0f, float yawDeg = 0.0f, float rollDeg = 0.0f, bool castShadows = true)
    {
    ...
    void CreateRoom()
    {
        LPENTITY floor = NULL_LPENTITY, wallLeft = NULL_LPENTITY, wallRight = NULL_LPENTITY, wallBack = NULL_LPENTITY;
        MakeRenderable(floor, g_cubeMesh, g_floorMat, "Floor", 0.0f, -0.1f, 0.0f, kRoomHalfW * 2.0f, 0.2f, kRoomDepth);
        MakeRenderable(wallLeft, g_cubeMesh, g_wallMat, "WallLeft", -kRoomHalfW, kRoomHalfH * 0.5f, 0.0f, 0.2f, kRoomHalfH, kRoomDepth);
        MakeRenderable(wallRight, g_cubeMesh, g_wallMat, "WallRight", kRoomHalfW, kRoomHalfH * 0.5f, 0.0f, 0.2f, kRoomHalfH, kRoomDepth);
        MakeRenderable(wallBack, g_cubeMesh, g_wallMat, "WallBack", 0.0f, kRoomHalfH * 0.5f, kRoomDepth * 0.5f, kRoomHalfW * 2.0f, kRoomHalfH, 0.2f);
    }
    ...
    void CreateSceneObjects()
    {
        ...
        for (size_t i = 0; i < spherePos.size(); ++i)
        {
            ...
            MakeRenderable(s, g_sphereMesh, (i % 2 == 0) ? g_sphereMatA : g_sphereMatB, "SceneSphere",
                spherePos[i][0], spherePos[i][1], spherePos[i][2], 1.1f, 1.1f, 1.1f);
        }
    }
    ...
    void InitScene()
    {
        ...
        if (Engine::_::renderer->SupportsTextureFormat(GDXTextureFormat::RGBA16_FLOAT))
        {
            ...
            Engine::_::renderer->SetGTAO(1280, 720, 0.1f, 100.0f, 18.0f, 1.5f, 1.0f, 1.5f);
        }
...
int main()
{
    if (!Engine::Graphics(Engine::Renderer::DX11, 1280, 720, "Room - GTAO", 0.02f, 0.02f, 0.04f, true))
        return 1;
