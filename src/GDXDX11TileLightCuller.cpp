...
// Init / Shutdown
// ---------------------------------------------------------------------------
bool GDXDX11TileLightCuller::Init(ID3D11Device* device, ID3D11ComputeShader* computeShader)
{
    if (!device || !computeShader) return false;
    m_cs = computeShader;
    m_cs->AddRef();
    if (!CreateStaticBuffers(device))
    {
        m_cs->Release();
        m_cs = nullptr;
        return false;
...
void GDXDX11TileLightCuller::Shutdown()
{
    ReleaseTileBuffers();
...
// ---------------------------------------------------------------------------
bool GDXDX11TileLightCuller::CreateStaticBuffers(ID3D11Device* device)
{
...
// ---------------------------------------------------------------------------
bool GDXDX11TileLightCuller::EnsureSize(ID3D11Device* device, uint32_t w, uint32_t h)
{
...
bool GDXDX11TileLightCuller::CreateTileBuffers(
...
void GDXDX11TileLightCuller::ReleaseTileBuffers()
{
    auto sr = [](auto*& p) { if (p) { p->Release(); p = nullptr; } };
...
// ---------------------------------------------------------------------------
void GDXDX11TileLightCuller::UploadLights(ID3D11DeviceContext* ctx, const FrameData& frame)
{
...
// Dispatch
// ---------------------------------------------------------------------------
void GDXDX11TileLightCuller::Dispatch(
...
// ---------------------------------------------------------------------------
void GDXDX11TileLightCuller::BindForPS(ID3D11DeviceContext* ctx)
{
    if (!ctx) return;
...
void GDXDX11TileLightCuller::UnbindFromPS(ID3D11DeviceContext* ctx)
{
    if (!ctx) return;
...
void GDXDX11TileLightCuller::UploadPSInfoCB(
