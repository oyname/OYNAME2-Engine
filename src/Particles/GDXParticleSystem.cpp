...
// ============================================================
//  Helpers: column extraction from view matrix
//
//  A DirectXMath LH view matrix (XMMatrixLookAtLH) stored in
//  our row-major Matrix4 (_11.._44) looks like:
//
//    _11=Rx _12=Ux _13=Fx _14=0
//    _21=Ry _22=Uy _23=Fy _24=0
//    _31=Rz _32=Uz _33=Fz _34=0
//    _41=tx _42=ty _43=tz _44=1
//
//  where R/U/F are the camera's right/up/forward in WORLD space.
//  So world-space Right = column 0 = {_11, _21, _31}.
//  GDX::GetMatrixRight() returns row 0 = {_11,_12,_13} which
//  is only correct for TRS world matrices, NOT view matrices.
//  We extract columns directly.
// ============================================================
...
//  Per-frame Update
// ============================================================
void GDXParticleSystem::Update(float deltaMs, const Matrix4& viewMat)
{
    ...
    for (int b = 0; b < 2; ++b)
    {
        if (m_instances[b].capacity() < particleCount)
            m_instances[b].reserve(particleCount);
        m_instances[b].clear();
    }
    ...
    // --- 3. Camera basis from view matrix (column extraction) ---
    m_camRight = ViewRight(viewMat);
    m_camUp = ViewUp(viewMat);

    // --- 4. Simulate + build instances ---
    m_spawnQueue.clear();
    ...
    for (size_t i = 0; i < n; )
    {
        ...
        BuildInstance(p, type.msh, nowMs);
        ++m_renderedCount;
        ++i;
    }

    // --- 5. Append trail spawns ---
...
// ============================================================
void GDXParticleSystem::BuildInstance(const GDXParticle& p, int blendMode, int nowMs)
{
    const GDXParticleTypeInternal& t = m_types[p.type];
    ...
    m_instances[blendMode].push_back(inst);
}
