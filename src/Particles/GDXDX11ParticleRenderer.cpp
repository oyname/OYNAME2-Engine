...
//  Render
// ============================================================
void GDXDX11ParticleRenderer::Render(const GDXParticleSystem&    sys,
                                      const ParticleRenderContext& ctx)
{
    ...
    // --- Alpha blend pass ---
    {
        const auto& inst = sys.GetInstances(0);
        if (!inst.empty())
        {
            m_ctx->OMSetBlendState(m_blendAlpha, bf, 0xFFFFFFFF);
            UploadAndDrawInstanced(0, inst);
        }
    }

    // --- Additive blend pass ---
    {
        const auto& inst = sys.GetInstances(1);
        if (!inst.empty())
        {
            m_ctx->OMSetBlendState(m_blendAdd, bf, 0xFFFFFFFF);
            UploadAndDrawInstanced(1, inst);
        }
    }
                                                       ...
                                                       const std::vector<ParticleInstance>& instances)
{
    ...
    if (n > m_maxInstances)
    {
        DBWARN(GDX_SRC_LOC, "UploadAndDrawInstanced: instance buffer overflow — clamped. Increase maxParticles.");
        n = m_maxInstances;
    }
